...
- `lib/s3.ts`: S3 클라이언트 설정 유틸리티 (이미 존재했음, 검토 완료)
- `README.md`: S3 설정 가이드 링크 추가

### 문서 추가
- `docs/S3_SETUP_GUIDE.md`: 상세한 S3 설정 가이드 (384줄)
  - S3 버킷 생성 방법
  - IAM 사용자 생성 및 권한 설정
  - 환경 변수 설정
  - 버킷 정책 설정
...
- ✅ 로컬 fallback 동작 확인 (S3 미설정 시)
- ✅ 에러 처리 확인 (권한 오류, 버킷 오류 등)
- ✅ 이미지 URL 정상 반환 확인
...
### S3 버킷 설정
- ✅ 버킷 정책에서 `images/*` 경로 공개 읽기 권한 설정 완료
- ✅ IAM 사용자 권한 설정 완료
- ✅ CORS 설정 완료
...
- ✅ 서버 확장성 개선 (로컬 파일 시스템 의존 제거)
- ✅ CDN을 통한 빠른 이미지 로딩 (CloudFront 지원)
- ✅ 개발 환경과 프로덕션 환경 분리
- ✅ 이미지 URL 공개 접근 가능 (블로그/위키에 적합)
...
- [ ] CloudFront CDN 설정 (선택사항, 성능 최적화)
- [ ] 이미지 최적화 (리사이징, 압축 등)
- [ ] Presigned URL 구현 (비공개 이미지가 필요한 경우)
